...
type ('c_A, 'c_S) travel2_C = ('c_A, 'c_S) travel2_C_1
and ('c_A, 'c_S) travel2_C_1 =
  [`connect of [`A of 'c_A * [`msg of unit data *
    ('c_A, 'c_S) travel2_C_2 sess]]]
and ('c_A, 'c_S) travel2_C_2 =
  [`send of [`A of 'c_A *
    [`query of string data *
      [`recv of [`A of 'c_A * [`quote of int data *
        ('c_A, 'c_S) travel2_C_2 sess]]] sess
    |`accpt of unit data *
      [`recv of [`A of 'c_A * [`port of int data *
        [`connect of [`S of 'c_S * [`msg of unit data *
          [`send of [`S of 'c_S * [`pay of string data *
            [`recv of [`S of 'c_S * [`confirm of int data *
              [`send of [`A of 'c_A * [`ack of int data *
                [`close] sess]]] sess]]] sess]]] sess]]] sess]]] sess
    |`reject of unit data *
      [`close] sess]]]
type ('c_C, 'c_S) travel2_A = ('c_C, 'c_S) travel2_A_1
and ('c_C, 'c_S) travel2_A_1 =
  [`accept of [`C of 'c_C * [`msg of unit data *
    ('c_C, 'c_S) travel2_A_2 sess]]]
and ('c_C, 'c_S) travel2_A_2 =
  [`recv of [`C of 'c_C *
    [`query of string data *
      [`send of [`C of 'c_C * [`quote of int data *
        ('c_C, 'c_S) travel2_A_2 sess]]] sess
    |`accpt of unit data *
      [`connect of [`S of 'c_S * [`msg of unit data *
        [`recv of [`S of 'c_S * [`port of int data *
          [`send of [`C of 'c_C * [`port of int data *
            [`recv of [`C of 'c_C * [`ack of int data *
              [`close] sess]]] sess]]] sess]]] sess]]] sess
    |`reject of unit data *
      [`close] sess]]]
type ('c_C, 'c_A) travel2_S = ('c_C, 'c_A) travel2_S_1
and ('c_C, 'c_A) travel2_S_1 =
  [`accept of [`A of 'c_A * [`msg of unit data *
    [`send of [`A of 'c_A * [`port of int data *
      [`accept of [`C of 'c_C * [`msg of unit data *
...
type travel2 = ShmemMPSTChanenl__ of Shmem.MPSTChannel.t
let create_shmem_channel : unit -> travel2 = fun () ->
  ShmemMPSTChanenl__(Shmem.MPSTChannel.create ~acceptor_role:"role_C" ~connector_roles:["role_A";; "role_S"])


module C = struct
  let initiate : unit -> 'c. ('c, 'c, ('c_A, 'c_S) travel2_C sess) monad =
  ...
  module A = struct
    module Make(X:sig
        ...
        val write_accpt : conn -> [>`accpt of unit data * 'p sess] -> unit io
        val write_reject : conn -> [>`reject of unit data * 'p sess] -> unit io
        val write_ack : conn -> [>`ack of int data * 'p sess] -> unit io
        val read_quote : conn -> [`quote of int data * 'p0] io
        val read_port : conn -> [`port of int data * 'p0] io
      end) = struct
      type conn = X.conn
      ...
      let accpt : 'p. ([>`accpt of unit data * 'p sess], X.conn, unit data * 'p sess) label =
      ...
      let ack : 'p. ([>`ack of int data * 'p sess], X.conn, int data * 'p sess) label =
        {_pack_label=(fun payload -> `ack(payload)); _send=X.write_ack}
      let receive_quote  : type p0. ([`quote of int data * p0], X.conn) labels =
        {_receive=X.read_quote}
      let receive_port  : type p0. ([`port of int data * p0], X.conn) labels =
        {_receive=X.read_port}
    end
    ...
    module Raw = struct
      include Make(struct
          ...
          let write_ack = Shmem.Raw.send
          let read_quote = Shmem.Raw.receive
          let read_port = Shmem.Raw.receive
        end)
    end
  end
  module S = struct
...
module A = struct
  let initiate : unit -> 'c. ('c, 'c, ('c_C, 'c_S) travel2_A sess) monad =
  ...
  module C = struct
    module Make(X:sig
        ...
        val write_port : conn -> [>`port of int data * 'p sess] -> unit io
        val read_msg : conn -> [`msg of unit data * 'p0] io
        val read_query_or_accpt_or_reject : conn -> [`query of string data * 'p0|`accpt of unit data * 'p1|`reject of unit data * 'p2] io
        val read_ack : conn -> [`ack of int data * 'p0] io
      end) = struct
      type conn = X.conn
      ...
      let port : 'p. ([>`port of int data * 'p sess], X.conn, int data * 'p sess) label =
        {_pack_label=(fun payload -> `port(payload)); _send=X.write_port}
      let receive_msg  : type p0. ([`msg of unit data * p0], X.conn) labels =
        {_receive=X.read_msg}
      let receive_query_or_accpt_or_reject  : type p0 p1 p2. ([`query of string data * p0|`accpt of unit data * p1|`reject of unit data * p2], X.conn) labels =
        {_receive=X.read_query_or_accpt_or_reject}
      let receive_ack  : type p0. ([`ack of int data * p0], X.conn) labels =
        {_receive=X.read_ack}
    end
...
module S = struct
  let initiate : unit -> 'c. ('c, 'c, ('c_C, 'c_A) travel2_S sess) monad =
