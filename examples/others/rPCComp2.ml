...
let accept_C : 'pre 'post. (rPCComp2,[`Implicit]) channel -> bindto:(empty, rPCComp2_C sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__accept ~myname:"role_C" ~cli_count:3 ch

let connect_S1 : 'pre 'post. (rPCComp2,[`Implicit]) channel -> bindto:(empty, rPCComp2_S1 sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__connect ~myname:"role_S1" ch
let connect_S2 : 'pre 'post. (rPCComp2,[`Implicit]) channel -> bindto:(empty, rPCComp2_S2 sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__connect ~myname:"role_S2" ch
let connect_S3 : 'pre 'post. (rPCComp2,[`Implicit]) channel -> bindto:(empty, rPCComp2_S3 sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__connect ~myname:"role_S3" ch

let new_channel_rPCComp2 : unit -> (rPCComp2,[`Implicit]) channel = new_channel
