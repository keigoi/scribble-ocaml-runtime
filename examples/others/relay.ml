...
let accept_R1 : 'pre 'post. (relay,[`Implicit]) channel -> ('c, 'c, relay_R1 sess) monad =
  fun ch ->
  Internal.__accept ~myname:"role_R1" ~cli_count:2 ch

let connect_R2 : 'pre 'post. (relay,[`Implicit]) channel -> ('c, 'c, relay_R2 sess) monad =
  fun ch ->
  Internal.__connect ~myname:"role_R2" ch
let connect_R3 : 'pre 'post. (relay,[`Implicit]) channel -> ('c, 'c, relay_R3 sess) monad =
  fun ch ->
