...
let initiate_C : 'pre 'post. (travel_Sec3,[`Explicit]) channel -> ('c, 'c, travel_Sec3_C sess) lin_match =
  fun ch ->
  Internal.__initiate ~myname:"role_C" ch
let initiate_A : 'pre 'post. (travel_Sec3,[`Explicit]) channel -> ('c, 'c, travel_Sec3_A sess) lin_match =
  fun ch ->
  Internal.__initiate ~myname:"role_A" ch
let initiate_S : 'pre 'post. (travel_Sec3,[`Explicit]) channel -> ('c, 'c, travel_Sec3_S sess) lin_match =
  fun ch ->
