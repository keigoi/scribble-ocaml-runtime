...
let accept_S1 : 'pre 'post. (game1Proto,[`Implicit]) channel -> bindto:(empty, game1Proto_S1 sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__accept ~myname:"role_S1" ~cli_count:2 ch

let connect_Client : 'pre 'post. (game1Proto,[`Implicit]) channel -> bindto:(empty, game1Proto_Client sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__connect ~myname:"role_Client" ch
let connect_S2 : 'pre 'post. (game1Proto,[`Implicit]) channel -> bindto:(empty, game1Proto_S2 sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
