...
let initiate_C : 'pre 'post. (travelAgency_Sec2,[`Explicit]) channel -> bindto:(empty, travelAgency_Sec2_C sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__initiate ~myname:"role_C" ch
let initiate_A : 'pre 'post. (travelAgency_Sec2,[`Explicit]) channel -> bindto:(empty, travelAgency_Sec2_A sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
  Internal.__initiate ~myname:"role_A" ch
let initiate_S : 'pre 'post. (travelAgency_Sec2,[`Explicit]) channel -> bindto:(empty, travelAgency_Sec2_S sess, 'pre, 'post) slot -> ('pre,'post,unit) monad =
  fun ch ->
